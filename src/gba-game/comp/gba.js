...
GameBoyAdvance.prototype.setCanvas = function(canvas) {
	...
	if (canvas.offsetWidth != 240 || canvas.offsetHeight != 160) {
		...
		let targetContext = canvas.getContext('2d');
...
GameBoyAdvance.prototype.loadRomFromFile = function(romFile, callback) {
	...
	// reader.onload = function(e) {
		let result = self.setRom(romFile);
		if (callback) {
			callback(result);
		}
	// }
...
GameBoyAdvance.prototype.downloadSavedata = function() {
	...
	if (window.URL) {
		let url = window.URL.createObjectURL(new Blob([sram.buffer], { type: 'application/octet-stream' }));
		window.open(url);
	} else {
		let data = this.encodeBase64(sram.view);
...
GameBoyAdvance.prototype.retrieveSavedata = function() {
	try {
		let storage = window.localStorage;
		let data = storage[this.SYS_ID + '.' + this.mmu.cart.code];
		if (data) {
